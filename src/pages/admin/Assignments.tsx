...
function looksLikeTimestamp(str: string): boolean {
  ...
  const datePatterns = [
    ...
    /student-\d+-\d+-\d+/i, // student-2-10-2026
  ];
  return datePatterns.some(pattern => pattern.test(str.trim()));
}

/**
 * Checks if a name looks auto-generated
 */
function isAutoGeneratedName(name: string): boolean {
  if (!name) return true;
  // Check for "Student" followed by number pattern
  if (/Student\s+\d+/.test(name)) return true;
  // Check for timestamp patterns
  if (looksLikeTimestamp(name)) return true;
  return false;
}

/**
 * Gets a clean display name for a student
 * Priority: Real name > Role + Country > Email-based identifier
 */
function getStudentDisplayName(student: StudentRow): string {
  const { full_name, email, target_role, country } = student;

  // 1. Use full_name if it's a real name (not auto-generated or timestamp)
  if (full_name && !isAutoGeneratedName(full_name)) {
    return full_name;
  }

  // 2. Generate a readable identifier from role + country
  if (target_role || country) {
    const role = target_role ? getShortRole(target_role) : "Student";
    const loc = country || "";
    if (loc) {
      return `${role} from ${loc}`;
    }
    return role;
  }

  // 3. Fallback to email-based display
  if (email) {
    // For survey.local emails, just show "Survey Respondent"
    if (email.includes("@survey.local")) {
      return "Survey Respondent";
    }
    // For real emails, show the prefix cleaned up
...
/**
 * Gets a short identifier for display (for use alongside the name)
 */
function getStudentIdentifier(student: StudentRow): string | null {
  // If we have a real email (not survey.local), show it
  if (student.email && !student.email.includes("@survey.local")) {
    return student.email;
  }
  // Otherwise show the survey timestamp if available
  return null;
}
  ...
  const handleAssign = async (trainerId: string, trainerName: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${displayName} has been assigned to ${trainerName}.`,
      });
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            {/* Only show real emails, not auto-generated survey.local */}
            {student.email && !student.email.includes("@survey.local") && (
            ...
            <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
              ...
              {student.weekly_hours && (
                <span className="text-muted-foreground">• {student.weekly_hours}</span>
              )}
              {student.country && !displayName.includes(student.country) && (
                <span className="text-muted-foreground">• {student.country}</span>
              )}
            </div>
          </div>
