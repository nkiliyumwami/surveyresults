  ...
  useEffect(() => {
    async function fetchFeatured() {
      const { data } = await supabase
        .from("students")
        .select(
          "id, full_name, country, certification_name, certification_date, credly_url, profile_image_url"
        )
          ...
          {students.map((s, i) => (
            ...
            >
              ...
              {s.certification_name && (
                <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyber-amber/15 border border-cyber-amber/30">
              ...
              {s.credly_url && (
                <a
                  ...
                  className="mt-3 inline-flex items-center gap-1.5 text-sm text-cyber-amber hover:text-cyber-amber/80 transition-colors"
                >
                  Verify on Credly
